import "phaser";
var space;
export class p1wScene extends Phaser.Scene {
    constructor() {
        super({
            key: "P1W"
        });
    ...
    create(): void {
        ...
        var player1scoret = this.add.text(100, 300, 'Player 1 wins!', { fontSize: '20px', color: '#FFFFFF' });
        var player2scoret = this.add.text(450, 300, 'Press Space to play again!', { fontSize: '20px', color: '#FFFFFF' });
    }
    update(time): void {
        if (space.isDown) {
